...
def get_number_of_characters_without_blank(filename):
    ...
    contents_without_blank = []

    for i in contents:
        if i != ' ' and i != '\n' and i != '\t':
            contents_without_blank.append(i)
...
def get_number_of_target_words(filename, target_words):
    ...
    # 2750
    # ===Modify codes below=============
    contents = get_file_contents(filename).lower()
    number = 0
    word = ''
    if target_words.lower() in contents:
        for i in contents:
            if target_words.lower() in word:
                number += 1
                word = ''
            else:
                word += i
