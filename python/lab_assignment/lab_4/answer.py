...
def merge_and_sort(list_data_a, list_data_b):
    ...
    # 한 줄 이상의 코드로 작성 가능하나,
    # 반드시 결과 값을 result 변수에 할당하여 반환
    merge_list = list_data_a + list_data_b

    result = sorted(merge_list)
...
def number_of_cases(list_data):
    ...
    # 한 줄 이상의 코드로 작성 가능하나,
    # 반드시 결과 값을 result 변수에 할당하여 반환
    result = sorted(list(set([str(i) + str(j) for j in list_data for i in list_data])))

    # result = []
    # for i in list_data:
    #     for j in list_data:
    #         if str(i)+str(j) not in result:
    #             result.append(str(i)+str(j))
    # # ==================================
    return result

print(number_of_cases([1,1,2,3]))
