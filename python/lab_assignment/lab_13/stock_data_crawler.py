...
def get_stock_data(url_address):
    ...
    for i in stock_data_string:
        if i == ',' or i == '\n':
            stock_line.append(items)
            items = ''
        else:
            items += i
        if i == '\n':
            stock_data_list.append(stock_line)
            stock_line = []
...
def get_attribute_data(stock_data, attribue, year=None, month=None):
    ...
    header = get_header_data(stock_data)
    index = 999 # default value
    result = []
    ...
    for i, v in enumerate(header):
        if attribue.lower() == v.lower():
            index = i

    for values in stock_data:
        if index == 999:
            result = stock_data
        else:
...
if __name__=="__main__":
    main()
    stock_data = get_stock_data('http://finance.google.com/finance/historical?q=KRX:005930&startdate=2013-01-01&enddate=2015-12-30&output=csv')
    print(get_attribute_data(stock_data, 'open'))
